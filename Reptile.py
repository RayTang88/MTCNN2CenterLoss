...
while(n<1000):
    ...
    #获取正则匹配到的结果，返回list
    s = p.findall(html)
    if not os.path.isdir("/home/ray/datasets/centerloss/data/{}".format(name)):
        os.makedirs("/home/ray/datasets/centerloss/data/{}".format(name))
    with open("/home/ray/datasets/centerloss/data/{}.txt".format(name), "a") as f:
        #获取图片
        for i in s:
            print(i)
            ...
            f.write("\n")
            #保存图片
            urllib.request.urlretrieve(i, "/home/ray/datasets/centerloss/data/{name}/{num}.jpg".format(name=name, num=j))
            j += 1
        f.close()
